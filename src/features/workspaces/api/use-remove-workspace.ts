...
interface Options {
  onSuccess?: (data: RemoveWorkspaceResponse) => void
...
export const useRemoveWorkspace = () => {
  const [data, setData] = useState<RemoveWorkspaceResponse | null>(null)
  ...
  const mutate = useCallback(
    async (values: RemoveWorkspaceRequest, options?: Options) => {
      try {
        setData(null)
